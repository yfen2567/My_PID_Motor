...
基于 STM32F103C8Tx、STM32 HAL 和 FreeRTOS 的直流电机速度控制项目。v2.2 在既有控制与诊断基线上补充了异步 PID 参数持久化、命令回归和启停回归的实机证据；v2.3.1 在此基础上增加 ControlTask 时序可观测性。

## 1. 项目状态

当前分支为 `v2.3.1-timing-observability`。v2.2 已完成参数持久化、非法/越界命令和 status/run/stop/restart 三项实机回归；v2.3.1 已完成 M0-M3 基础字段回归和新增聚合字段冒烟验证，M4 GPIO 波形验证延期。本状态不代表工业级验证或 GitHub Release 已发布。
...
- [M2 电机运行时序报告](reports/v2.3_m2_timing_run_report.md)
- [M3 命令压力时序报告](reports/v2.3_m3_timing_command_pressure_report.md)
- [聚合字段冒烟报告](reports/v2.3_timing_aggregate_smoke_report.md)
...
| `timing stats` | 查询当前时序与累计聚合统计 | 返回 `TIMING_STATS ...`；不进入控制队列 |
...
其中当前样本字段来自最近一次有效周期，`sample_count`、min/max 和 `max_abs_jitter_us` 为自 ControlTask 第一份有效样本以来的累计聚合统计。M0-M3 历史日志是在聚合字段加入前生成的；新增字段由独立冒烟测试验证。
